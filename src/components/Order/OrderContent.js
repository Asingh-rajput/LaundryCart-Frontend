...
function OrderContent() {
    ...
    useEffect(() => {
        fetch("https://laundry-crt-backend.onrender.com/getorder", {
            method: "GET",
            headers: {
    ...
    const deleteItem = (id) => {
        fetch("https://laundry-crt-backend.onrender.com/deleteorder", {
            method: "delete",
            headers: {
                "Authorization": "Bearer " + localStorage.getItem("jwt"),
                "Content-Type": "application/json"
    ...
    const findServiceArray = (id) => {
        fetch("https://laundry-crt-backend.onrender.com/findorder", {
            method: "POST",
            headers: {
                "Authorization": "Bearer " + localStorage.getItem("jwt"),
                "Content-Type": "application/json"
    ...
    return <>
        ...
        {/* {myorders && } */}
        {myorders && <div className="myorder-header">
            ...
            }} style={{ position: "absolute", top: "-30px", left: "1150px", backgroundColor: "transparent", cursor: "pointer" }}>Create</button>
            {data.map((item) => {
                return <div key={item._id} className="order-list">
                    ...
                    <div className="align r p"><span>₹</span>{item.price}</div>
                    <div className="align s">{item.service[0]}</div>

                    <div style={{ cursor: "pointer" }} onClick={() => {
                        findServiceArray(item._id)
                        showOrderhistory(true)
                    }} className="align dot"><img className="eye" src={eye} alt="eye.svg" /></div>

                    <div className="align del"><img onClick={() => {
                        showDeleteMessage(true)
                        setDeleteById(item._id)
                    }} src="https://icon-library.com/images/delete-icon/delete-icon-13.jpg" alt="delete.svg" />
                    </div>
                </div>
            })}
        </div>}
        <OrderSideBar />
        {deleteMessage && <div className="message-box">
            <img style={{ float: "right", height: "14px", cursor: "pointer" }} src={cross} onClick={() => {
                showDeleteMessage(false)
            }} alt="cross" />
            <img style={{ height: "150px", width: "150px" }} src={cancel} alt="check.jpg" />
            <p style={{ color: "#0A1F44", fontSize: "24px", fontWeight: "bold" }}>Click on delete button to cancel your order.</p>
        ...
        <OrderFooter />
        {orderHistory && <div className="view-summary">
            ...
            <div style={{backgroundColor:"#FFFFFF"}}><img style={{width:"100%"}} src={status} alt="pick-up-detail" /></div>
